...
const db_ops = {
  ...
  get_comments_by_album: db.prepare(
  ...
  get_comment: db.prepare("SELECT id, album_id, user_id, content, created_at FROM comments WHERE id = ?;"),
  delete_comment: db.prepare("DELETE FROM comments WHERE id = ?;"),
};

function addComment(albumId, userId, content) {
  const now = Date.now();
  return db_ops.insert_comment.get(albumId, userId, content, now);
}

function getCommentsForAlbum(albumId) {
  return db_ops.get_comments_by_album.all(albumId);
}
...
function deleteComment(id) {
  return db_ops.delete_comment.run(id);
}
